...
export function AreasClient({ initial }: { initial: Area[] }) {
  ...
  const [upsertState, upsertAction, isSaving] = useActionState(upsertArea, init);
  const [deleteState, deleteAction, isDeleting] = useActionState(deleteArea, init);
  ...
  function handleToggle(item: Area) {
    setTogglingId(item.id);
    startTransition(async () => {
      const next = !item.active;
      setItems((prev) => prev.map((i) => (i.id === item.id ? { ...i, active: next } : i)));
      const res = await toggleAreaActive(item.id, next);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="flex items-center gap-2 text-lg font-semibold">
            <MapPin className="h-5 w-5 text-accent-600" /> Áreas
          </h2>
      ...
      <AlertDialog open={Boolean(deleting)} onOpenChange={(open) => !open && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {`Confirma a exclusão de "${deleting?.name}"? Unidades vinculadas a essa área impedirão a exclusão.`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function AreaForm({
  ...
  const { values, setValue, errors, markTouched, isValid, isDirty, validateAll } =
    useLiveValidation<FormValues>(areaSchema, initial);
  ...
  return (
    <FormDialog
      open
      title={item ? "Editar Área" : "Nova Área"}
      subtitle={item ? "Atualize os dados da área" : "Cadastre uma nova área regional"}
      isDirty={isDirty}
      onClose={onClose}
      onSubmit={submit}
      isSaving={isSaving}
      ...
      submitLabel={item ? "Salvar alterações" : "Criar Área"}
      size="sm"
    >
      <Field
        id="area-name"
        label="Nome"
        required
        helpText="Nome da área regional (ex: Pará-Amapá, Amazônia, Cerrado, Sul)."
        maxLength={80}
        value={values.name}
        error={errors.name}
      >
      ...
      <div className="rounded-lg border border-zinc-200 bg-zinc-50/50 p-3 dark:border-zinc-700 dark:bg-zinc-800/30">
        ...
        <p className="mt-1 pl-11 text-xs text-zinc-500">
          Áreas inativas não aparecem em dropdowns de novos cadastros.
        </p>
      </div>
    </FormDialog>
  );
